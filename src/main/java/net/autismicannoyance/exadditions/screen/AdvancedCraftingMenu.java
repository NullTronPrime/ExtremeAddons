...
public class AdvancedCraftingMenu extends AbstractContainerMenu {
    ...
    private void updateCraftingResult() {
        if (!level.isClientSide()) {
            SimpleContainer craftingContainer = new SimpleContainer(25);

            // Fill crafting container with current items
            for (int i = 0; i < 25; i++) {
                craftingContainer.setItem(i, blockEntity.getItemHandler().getStackInSlot(i));
            }

            // Find matching recipe
            Optional<AdvancedCraftingRecipe> recipe = level.getRecipeManager()
                    .getRecipeFor(ModRecipeTypes.ADVANCED_CRAFTING_TYPE.get(), craftingContainer, level);
            ...
            if (recipe.isPresent()) {
                ...
                blockEntity.getItemHandler().setStackInSlot(25, ItemStack.EMPTY);
            }
    ...
    // Custom result slot that handles crafting
    private class AdvancedCraftingResultSlot extends SlotItemHandler {
        ...
        public void onTake(Player pPlayer, ItemStack pStack) {
            // Handle crafting result take - consume ingredients
            if (!level.isClientSide()) {
                SimpleContainer craftingContainer = new SimpleContainer(25);
    ...
    // CREDIT GOES TO: diesieben07 | https://github.com/diesieben07/SeasonHolidays
